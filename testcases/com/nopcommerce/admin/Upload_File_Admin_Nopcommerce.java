...
public class Upload_File_Admin_Nopcommerce extends BaseTestNopcommerce {
	...
	public void Admin_01_Upload_File() {
		...
		verifyFalse(productDetailPage.isPictureUploadedSuccessByFileName(fileName));
		...
		verifyFalse(productDetailPage.isPictureDisplayed(clickToOrderIconTimes, alt, title));
		productSearchPage = productDetailPage.clickToSaveButton();
		verifyFalse(productSearchPage.isSuccessUploadFileTextDisplay());
		productSearchPage.enterToTextboxByName("Adobe Photoshop CS4", "SearchProductName");
		productSearchPage.clickToSearchButton();
		verifyFalse(productSearchPage.isPictureUploaded("adobe-photoshop-cs4", "Adobe Photoshop CS4"));
		productDetailPage = productSearchPage.clickToEditButton();
		productDetailPage.clickToExpandPanelByName("far fa-images");
		productDetailPage.clickToDeleteButtonAtPictureName();
		Assert.assertTrue(productDetailPage.isDeletedMessageDisplayed());
		productSearchPage = productDetailPage.clickToSaveButton();
		productSearchPage.enterToTextboxByName("Adobe Photoshop CS4", "SearchProductName");
		productSearchPage.clickToSearchButton();
		Assert.assertTrue(productSearchPage.isPictureUploaded("default-image", "Adobe Photoshop CS4"));
	}
